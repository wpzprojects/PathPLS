...
- `index.html` — página pública: buscador, pestañas (PLS-CADD / Otras acciones / PLS-POLE / De interés / Workflow / Ayuda), tarjetas de pasos con acordeón, lightbox de imágenes. Los paneles "De interés", "Workflow" (visor del PDF) y "Ayuda" (resumen por sección, tarjeta "Copia de seguridad" y tarjeta de contacto) están *hardcodeados* aquí. El botón de entrada a admin (ícono de sliders, clave `pls`, ver `js/app.js`) vive al final del panel "Ayuda".
...
- `js/vendor/pdf.min.js` + `pdf.worker.min.js` — PDF.js 3.11.174 local. Pestaña Workflow: `initPdf()` (en `app.js`) lo carga al abrir la pestaña y dibuja `assets/docs/PLS-CADD_Workflow.pdf` (19 láminas, material de Power Line Systems) página a página con IntersectionObserver; se usa PDF.js porque los iframes de PDF no funcionan en Android/iOS. Cada lámina toma su proporción real (`fitSlot`; el PDF mezcla 16:9, 4,8:1 y vertical 3:4). Tocar una lámina abre el zoom a pantalla completa (`#pdfz`): 100% = lámina completa en la ventana, +/− / Ctrl+rueda / doble clic / arrastre, y se **re-dibuja a la resolución del zoom** (`pzRender`, con `PZ_PIXELS` según dispositivo: 16M iOS, 32M táctil, 64M escritorio); máx 800%.
